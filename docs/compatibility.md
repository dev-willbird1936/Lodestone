...
| --- | --- | --- | --- | --- | --- |
| NeoForge host + adapter | Minecraft Java 1.21.1 | NeoForge 21.1.211, NeoGradle 7.1.38, Gradle 8.14 | integration-tested (packaged matrix); manually verified client-menu/control slice (post-v1 source) | `verification/evidence/compatibility-live-2026-07-12.md`, `verification/evidence/neoforge-1.21.1-client-live.md`, and `verification/evidence/neoforge-1.21.1-client-menu-live-2026-07-14.md` | Authenticated guarded navigation reached a fresh world through the two modern `Create New World` screens, and player look has exact readback. The bounded 120-second UI defaults are packaged; English vanilla labels, broad menu automation, containers/NBT, and broader mutation/event coverage remain open. This does not change immutable v1.0.0 bytes. |
| Fabric adapter | Minecraft Java 1.21.1 | Fabric Loader 0.16.10, Fabric API 0.116.9+1.21.1, Fabric Loom 1.10.5, Gradle 8.14 | integration-tested (packaged matrix); manually verified client-menu/control slice (post-v1 source) | `verification/evidence/compatibility-live-2026-07-12.md`, `verification/evidence/fabric-1.21.1-client-live.md`, and `verification/evidence/fabric-1.21.1-client-menu-live-2026-07-14.md` | Authenticated guarded navigation reached a fresh world through the two modern `Create New World` screens, and player look has exact readback. The world-list projection was partial, so only its unique visible target was selected. Bounded 120-second UI defaults are packaged; English vanilla labels, broad menu automation, containers/NBT, and broader mutation/event coverage remain open. This does not change immutable v1.0.0 bytes. |
| Fabric adapter | Minecraft Java 1.20.1 | Fabric Loader 0.15.11, Fabric API 0.92.2+1.20.1, Fabric Loom 1.10.5, Gradle 8.14 | integration-tested (packaged matrix); manually verified client-menu/control slice (post-v1 source) | `verification/evidence/compatibility-live-2026-07-12.md` and `verification/evidence/fabric-1.20.1-client-menu-live-2026-07-14.md` | Bounded overworld writes and loaded-entity queries are live-tested. Authenticated typed UI navigation reached a fresh world, and player look has exact readback. Bounded 120-second UI transition defaults avoid false post-commit quarantine during normal world load; a short lifecycle refresh may still report client capabilities unavailable. Containers/NBT and broader mod interoperability remain open. This does not change immutable v1.0.0 bytes. |
...
| Fabric adapter | Minecraft Java 26.2 | Fabric Loader 0.19.3, Fabric API 0.154.2+26.2, Fabric Loom 1.17.1, Gradle 9.5.1, Java 25 | integration-tested (packaged matrix) | `verification/evidence/fabric-26.2-live.md`, `verification/evidence/compatibility-live-2026-07-12.md`, and `verification/curseforge-profiles/lodestone-fabric-26.2-local.zip` | Native server/world/MCP path is live-tested; Vulkan is a client-only experimental graphics setting and is not exercised by a headless server |
| Forge host + adapter | Minecraft Java 1.21.1 | Forge 52.1.0, ForgeGradle 6.0.24, Gradle 8.14, Java 21 | integration-tested (packaged matrix) | `verification/evidence/compatibility-live-2026-07-12.md`, `verification/curseforge-profiles/lodestone-forge-1.21.1-local.zip` | Same bounded native slice; adapter source is compiled against the exact Forge 52.1.0 userdev artifact |
