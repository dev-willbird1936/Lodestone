 ...
 *
 * <p>Ported from {@code NeoForgeGoalSupervisor} for the {@code minecraft.goal.navigation.safe-waypoint}
 * capability; behavior is bit-for-bit identical to the NeoForge 1.21.1 original.
 */
final class FabricGoalSupervisor {
    ...
    private final FabricSuffocationReflex suffocationReflex = new FabricSuffocationReflex();
    private int suffocationEscapeTicks;

    FabricGoalSupervisor(FabricGoalPolicy policy, Collection<String> actions,
    ...
    boolean tickImmediateSurvival(Minecraft client) {
        ...
        var decision = FabricSurvivalInvariant.decide(player.isDeadOrDying(), player.getHealth(),
                false, player.isInWater(), player.getAirSupply(), player.getMaxAirSupply());
        if (decision != FabricSurvivalInvariant.Action.WATER_RETREAT) {
        ...
        var route = FabricSafePathPlanner.findWaterRetreat(client.level, player, player.blockPosition(),
        ...
        if (route.size() >= 2) {
            ...
            var allowHorizontal = FabricSurvivalInvariant.allowHorizontalWaterInput(true, horizontal);
    ...
    private boolean tickSuffocationEscape(Minecraft client, LocalPlayer player) {
        ...
        if (suffocationEscapeTicks % 10 == 1) KeyMapping.click(client.options.keyAttack.getDefaultKey());
    ...
    private boolean escapeFireOrLava(Minecraft client, LocalPlayer player) {
        var route = FabricSafePathPlanner.findHazardRetreat(client.level, player, player.blockPosition(), policy);
        if (route.size() >= 2) {
    ...
    private boolean recoverObstructionOrStall(Minecraft client, LocalPlayer player,
        ...
        if (policy.obstructionMiningEnabled() && !obstructionClearUsedInCluster
            ...
            var snapshot = FabricWorldSnapshot.capture(client.level, policy, player);
            if (!state.isAir() && state.getFluidState().isEmpty()
    ...
    private boolean tickObstructionClear(Minecraft client, LocalPlayer player) {
        ...
        if (hit instanceof BlockHitResult blockHit && blockHit.getBlockPos().equals(obstructionClearTarget)) {
            if (obstructionClearTicks % 10 == 1) KeyMapping.click(client.options.keyAttack.getDefaultKey());
            client.options.keyAttack.setDown(true);
    ...
    private boolean hasBufferedRetreat(Minecraft client, LocalPlayer player,
        ...
        var feet = player.blockPosition();
        var arrival = new FabricSafePathPlanner.ArrivalSpec(0.8, 0.8);
        for (var direction : Direction.Plane.HORIZONTAL) {
            for (var dy : new int[]{0, 1, -1}) {
                var candidate = feet.relative(direction).offset(0, dy, 0);
                if (!snapshot.bufferedWalkable(candidate)) continue;
                var path = FabricSafePathPlanner.find(client.level, player, feet, candidate, policy, arrival);
