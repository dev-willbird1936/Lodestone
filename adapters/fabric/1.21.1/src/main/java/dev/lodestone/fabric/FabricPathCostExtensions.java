...
/**
 * Extension point for path-edge cost terms beyond {@link FabricSafePathPlanner}'s own geometry,
 * tier, and fall-damage costs: high-safety mob/lava proximity and fall-through risk, and
 * adaptive-intelligence mine-vs-place cost.
 *
 * <p>Ported from {@code NeoForgePathCostExtensions} for the {@code minecraft.goal.navigation.safe-waypoint}
 * capability; behavior is bit-for-bit identical to the NeoForge 1.21.1 original.
 */
final class FabricPathCostExtensions {
    ...
    private FabricPathCostExtensions() { }

    static double additionalCost(FabricSafePathPlanner.EdgeContext ctx) {
        var cost = 0.0;
    ...
    private static double mobProximityCost(FabricSafePathPlanner.EdgeContext ctx) {
        var total = 0.0;
    ...
    private static double lavaProximityCost(FabricSafePathPlanner.EdgeContext ctx) {
        var total = 0.0;
    ...
    private static double fallThroughCost(FabricSafePathPlanner.EdgeContext ctx) {
        var level = ctx.level();
    ...
    private static double mineCost(FabricSafePathPlanner.EdgeContext ctx) {
        if (!ctx.policy().obstructionMiningEnabled()) return Double.POSITIVE_INFINITY;
        var total = 0.0;
        for (var target : ctx.mutationTargets()) {
            if (!FabricGoalActionGuard.canBreakObstruction(ctx.level(), ctx.player(), target, ctx.policy())) {
    ...
    private static double placeCost(FabricSafePathPlanner.EdgeContext ctx) {
     ...
     * acquire-planning for a block the player doesn't already own).
     */
