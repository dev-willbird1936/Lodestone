 ...
 *
 * <p>Ported from {@code NeoForgeWorldSnapshot} for the {@code minecraft.goal.navigation.safe-waypoint}
 * capability; behavior is bit-for-bit identical to the NeoForge 1.21.1 original. Trimmed of
 * {@code nearestSafeSurface}, which the ported navigation goal and its supervisor never call (it
 * only ever served other, out-of-scope goal actors on the NeoForge side).
 */
final class FabricWorldSnapshot {
    ...
    // Package-private: the ported navigation goal has no separate full observation capability, so
    // this threat scan bound (and the isThreat predicate below) is inlined here from
    // NeoForgeGoalObservation rather than porting that whole (out-of-scope) capability's class.
    ...
    private FabricWorldSnapshot(ClientLevel level, FabricGoalPolicy policy, LocalPlayer player) {
        this.level = level;
        this.policy = policy;
        this.player = player;
    ...
    static FabricWorldSnapshot capture(ClientLevel level, FabricGoalPolicy policy, LocalPlayer player) {
        return new FabricWorldSnapshot(level, policy, player);
    }
    ...
    /** Hostile/targeting-mob filter, mirroring {@code NeoForgeGoalObservation.isThreat}. */
    private static boolean isThreat(Mob mob, LocalPlayer player) {
        return mob.isAlive() && (mob.getTarget() == player || mob instanceof Monster);
    }
    ...
    boolean hazard(BlockPos position) {
        ...
        return !level.getFluidState(position).isEmpty()
                || FabricContactHazards.isDamageBlock(state.getBlock());
    }
     ...
     * support block must present a sturdy up-facing face to click against.
     */
