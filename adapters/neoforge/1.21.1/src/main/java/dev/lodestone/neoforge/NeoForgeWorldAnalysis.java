...
final class NeoForgeWorldAnalysis {
    ...
    static Operation operation(String capability) {
        return switch (capability) {
            ...
            default -> throw new IllegalArgumentException("unsupported world analysis capability: " + capability);
        };
    }
    ...
    static Map<String, Object> lightAnalysis(LightRequest request, String dimension,
        ...
        if (request.y() < minBuildHeight || lastY >= maxBuildHeight) {
            throw new IllegalArgumentException("light analysis region must stay within world build height");
        }
        ...
        for (var offsetY = 0; offsetY < request.sizeY(); offsetY += request.resolution()) {
            for (var offsetZ = 0; offsetZ < request.sizeZ(); offsetZ += request.resolution()) {
                for (var offsetX = 0; offsetX < request.sizeX(); offsetX += request.resolution()) {
                    ...
                    if (combined < 8) {
                        darkSpotCount++;
                        var darkSpot = new DarkSpot(x, y, z, sample.blockLight(), sample.skyLight(), combined);
        ...
        var distribution = Map.of(
                "wellLit", Map.of("count", summary.wellLitCount(), "percentage", summary.wellLitPercentage()),
                "dim", Map.of("count", summary.dimCount(), "percentage", summary.dimPercentage()),
                "dark", Map.of("count", summary.darkCount(), "percentage", summary.darkPercentage()));
        var suggestions = suggestionAccumulator.snapshot().stream()
                .map(suggestion -> Map.<String, Object>of(
                        ...
                        "suggestedSource", suggestion.suggestedSource(), "reason", suggestion.reason()))
                .toList();
    ...
    static final class LightAccumulator {
        ...
        LightSummary snapshot() {
            ...
            return new LightSummary(analyzedSamples, average, dark, percentage(dark, analyzedSamples),
                    dim, percentage(dim, analyzedSamples), wellLit, percentage(wellLit, analyzedSamples),
    ...
    private static final class SuggestionAccumulator {
        ...
        private void accept(DarkSpot spot) {
            ...
            var cell = new SuggestionCell(Math.floorDiv(spot.x(), 14), Math.floorDiv(spot.y(), 8),
                    Math.floorDiv(spot.z(), 14));
            if (!covered.add(cell)) {
                return;
            }
            if (spot.y() < minY + 3) {
