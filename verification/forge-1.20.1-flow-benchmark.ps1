...
param(
    ...
    [string] $LodestoneArtifact = (Join-Path $PSScriptRoot '..\hosts\forge\1.20.1\build\libs\lodestone-1.0.0.jar'),
    [string] $ClientRunDirectory = (Join-Path $PSScriptRoot '..\hosts\forge\1.20.1\run-artifact-client')
)
...
& (Join-Path $PSScriptRoot 'neoforge-keepfocus-flow-benchmark.ps1') `
    ...
    -BenchmarkName 'forge-1.20.1-client-flow' `
    -ReportPrefix 'forge-1.20.1-flow' `
    -ChatMarker '[Lodestone] Forge 1.20.1 flow benchmark marker' `
