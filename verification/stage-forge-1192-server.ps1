...
param(
    ...
    [string]$Root = (Join-Path $env:TEMP 'lodestone-matrix\forge-1.19.2'),
...
$forgeVersion = '1.19.2-43.5.2'
$forgeRoot = Join-Path $rootPath 'libraries\net\minecraftforge\forge\1.19.2-43.5.2'
...
@(
    ...
    'enable-query=false'
    'level-name=world'
    'motd=Lodestone Forge 1.19.2 acceptance server'
) | Set-Content -LiteralPath (Join-Path $rootPath 'server.properties') -Encoding ascii
Write-Output "Staged Forge 1.19.2 server at $rootPath"
