...
function Assert-BadTokenRejected([string]$uri) {
    ...
    try {
        Invoke-WebRequest -UseBasicParsing -Uri $uri -Method Post -Headers @{
            'X-Lodestone-Token' = 'invalid-spigot-matrix-token'
            'MCP-Protocol-Version' = '2025-11-25'
    ...
    Assert ($statusCode -eq 401) "Spigot bad-token request returned HTTP $statusCode instead of 401."
    Write-Host 'Spigot bad-token rejection verified (HTTP 401).'
}
