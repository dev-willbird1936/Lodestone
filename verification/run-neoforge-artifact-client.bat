...
call "%~dp0..\gradlew.bat" --no-daemon --console=plain -p "%~dp0neoforge-artifact-client" "-Dlodestone.port=%LODESTONE_PORT%" "-Dlodestone.token=%LODESTONE_TOKEN%" "-Dlodestone.permissions=%LODESTONE_PERMISSIONS%" runArtifactClient
exit /b %ERRORLEVEL%
