...
function Log($x){$x.timestampUtc=[DateTime]::UtcNow.ToString('o');$line=$x|ConvertTo-Json -Depth 50 -Compress;for($writeAttempt=0;$writeAttempt-lt 12;$writeAttempt++){try{Add-Content -LiteralPath $evidencePath -Value $line;return}catch [System.IO.IOException]{Start-Sleep -Milliseconds 100}};throw "evidence JSONL remained locked after bounded append retries: $evidencePath"}
...
function Cap([string]$cap,[hashtable]$inputArgs=@{},[string]$version='1.0'){
  for($attempt=0;$attempt-lt 8;$attempt++){$result=Call 'lodestone_capability_invoke' @{capability=$cap;capabilityVersion=$version;input=$inputArgs;dryRun=$false};if($result.status -eq 'ok'){return $result.output};if([string]$result.error.code -ne 'RATE_LIMIT_EXCEEDED'){return Must $result $cap};Start-Sleep -Milliseconds 450}
