...
param(
    ...
    [string] $LodestoneArtifact = (Join-Path $PSScriptRoot '..\hosts\fabric\1.19.2\build\libs\lodestone-1.0.0.jar'),
    [string] $ClientRunDirectory = (Join-Path $PSScriptRoot '..\hosts\fabric\1.19.2\run')
)
...
& (Join-Path $PSScriptRoot 'neoforge-keepfocus-flow-benchmark.ps1') `
    ...
    -BenchmarkName 'fabric-1.19.2-client-flow' `
    -ReportPrefix 'fabric-1.19.2-flow' `
    -ChatMarker '[Lodestone] Fabric 1.19.2 flow benchmark marker' `
