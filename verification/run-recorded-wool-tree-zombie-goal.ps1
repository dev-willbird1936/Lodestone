...
$stem = "minecraft-goal-wool-tree-zombie-defense-$Mode-$timestamp"
...
$goalText = 'load into a world in creative, build a tree manually with wool for leaves not too far away, put a zombie next to it, tp a bit away from the tree, give self a diamond sword, go into survival, then attempt to mine the tree but keeping eye out to defend itself from the zombie without hardcoding killing the zombie'
...
$report = [ordered]@{
    formatVersion = 1
    evidenceKind = 'recorded-clean-wool-tree-zombie-defense-goal'
    ...
    keepFocus = $true
    mode = $Mode
    taskId = 'creative.wool-tree-zombie-defense'
    goal = $goalText
    videoPath = $videoPath
    ...
    advisorCheckpoint = [ordered]@{
        status = 'UNAVAILABLE'
        reason = 'The one permitted read-only codex-advisor checkpoint timed out after 94 seconds without a final response; delegation forbids retrying or fabricating advice.'
    }
}
...
try {
    ...
    $initialize = Invoke-Rpc 'initialize' @{
        ...
        clientInfo = @{ name = 'lodestone-recorded-wool-tree-zombie-goal'; version = '1.0' }
    }
    ...
    $tasks = Require-SetupOk (Invoke-SetupTool 'minecraft_goal_tasks' @{ category = 'creative' }) 'minecraft_goal_tasks'
    $task = @($tasks.tasks | Where-Object { $_.id -eq 'creative.wool-tree-zombie-defense' }) | Select-Object -First 1
    if (-not $task) { throw 'creative.wool-tree-zombie-defense is absent from the live task catalog' }
    if (@($task.requiredCapabilities) -notcontains 'minecraft.goal.creative.wool-tree-zombie-defense') {
        throw 'creative defense task is missing its authentic NeoForge gameplay capability'
    }
    ...
    $state = $goalResponse.output.state.steps.'wool-tree-defense'
    if (-not $state) { throw 'goal result is missing state.steps.wool-tree-defense' }
    $report.goalPredicates = [ordered]@{
        freshWorld = [bool] $state.freshWorld
        creativeSetupMode = [bool] $state.creativeSetupMode
        manualTreeBuilt = [bool] $state.manualTreeBuilt
        manualPlacementInputOnly = [bool] $state.manualPlacementInputOnly
        trunkLogsPlaced = [int] $state.trunkLogsPlaced
        woolLeavesPlaced = [int] $state.woolLeavesPlaced
        zombieSetupComplete = [bool] $state.zombieSetupComplete
        teleportedAway = [bool] $state.teleportedAway
        teleportDistance = [double] $state.teleportDistance
        diamondSwordEquipped = [bool] $state.diamondSwordEquipped
        survivalMode = [bool] $state.survivalMode
        zombieObserved = [bool] $state.zombieObserved
        reactiveDefenseEvaluated = [bool] $state.reactiveDefenseEvaluated
        defensePolicyEvaluations = [int] $state.defensePolicyEvaluations
        threatDetections = [int] $state.threatDetections
        defensiveResponses = [int] $state.defensiveResponses
        defensiveAttacks = [int] $state.defensiveAttacks
        zombieKilledByReactiveDefense = [bool] $state.zombieKilledByReactiveDefense
        unconditionalKillRoutine = [bool] $state.unconditionalKillRoutine
        treeInitialBlocks = [int] $state.treeInitialBlocks
        treeMinedBlocks = [int] $state.treeMinedBlocks
        treeRemainingBlocks = [int] $state.treeRemainingBlocks
        ...
        defenseDiagnostics = [string[]] @($state.defenseDiagnostics)
        inputActions = [string[]] @($state.inputActions)
    }
    $requiredPredicates = @(
        [bool] $state.freshWorld,
        [bool] $state.creativeSetupMode,
        [bool] $state.manualTreeBuilt,
        [bool] $state.manualPlacementInputOnly,
        ([int] $state.trunkLogsPlaced -ge 3),
        ([int] $state.woolLeavesPlaced -ge 9),
        [bool] $state.zombieSetupComplete,
        [bool] $state.teleportedAway,
        ([double] $state.teleportDistance -ge 6.0),
        [bool] $state.diamondSwordEquipped,
        [bool] $state.survivalMode,
        [bool] $state.zombieObserved,
        [bool] $state.reactiveDefenseEvaluated,
        ([int] $state.defensePolicyEvaluations -ge 1),
        ([int] $state.threatDetections -ge 1),
        ([int] $state.defensiveResponses -ge 1),
        ([int] $state.defensiveAttacks -ge 1),
        ([int] $state.treeRemainingBlocks -eq 0),
        [bool] $state.fullTreeMined,
        [bool] $state.playerAlive,
        [bool] $state.setupCommandsUsed,
        ([int] $state.setupCommandCount -ge 8),
        [bool] $state.commandFeedbackSuppressed,
        [bool] $state.suppressInGameMessages,
        ([int] $state.inGameMessagesEmitted -eq 0),
        (-not [bool] $state.unconditionalKillRoutine),
        (-not [bool] $state.directMutationUsed)
    )
    if ($requiredPredicates -contains $false) {
        throw 'goal returned SUCCEEDED without every required build/defense/clean-gameplay predicate'
    }
    if (@($state.setupCommands | Where-Object { $_ -match 'setblock|fill|loot|replaceitem|data modify' }).Count -ne 0) {
        throw 'setup command trace contains a forbidden direct tree/inventory mutation command'
    }
    if (@($state.inputActions | Where-Object { $_ -like 'place:key.use:*' }).Count -lt 12) {
        throw 'input trace does not prove all twelve manual tree placements'
    }
    if (@($state.defenseDiagnostics | Where-Object { $_ -like 'threat-detected:*' }).Count -lt 1 -or
        @($state.inputActions | Where-Object { $_ -eq 'defense:key.attack-reactive' }).Count -lt 1) {
        throw 'defense trace does not prove a visibility/distance-gated reactive sword attack'
    }

    $chatComponentLines = [string[]] @(Select-String -LiteralPath $stdoutLog -Pattern '\[minecraft/ChatComponent\]' |
        ForEach-Object { $_.Line })
    $report.chatComponentLines = $chatComponentLines
    $report.cleanGameplayLogVerified = $chatComponentLines.Count -eq 0
    if ($chatComponentLines.Count -ne 0) {
        throw "clean gameplay was contaminated by client chat/status feedback: $($chatComponentLines -join ' | ')"
    }
