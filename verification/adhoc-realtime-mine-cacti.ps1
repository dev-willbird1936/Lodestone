...
$logDir = Join-Path $scriptRoot 'evidence\adhoc-realtime-mine-cacti'
...
if (-not (Get-NetTCPConnection -LocalPort $ModelPort -State Listen -ErrorAction SilentlyContinue)) {
    throw "expected an already-running model proxy on port $ModelPort - none found"
}
...
$initialize = Invoke-Rpc 'initialize' @{ protocolVersion = '2025-11-25'; clientInfo = @{ name = 'lodestone-adhoc-realtime-cacti'; version = '1.0' } }
...
Write-Host "Sending realtime minecraft_goal: mine 5 cacti (intelligence=$Intelligence, safety=$Safety)..."
