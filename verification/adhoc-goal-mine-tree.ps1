...
$logDir = Join-Path $scriptRoot 'evidence\adhoc-goal-mine-tree'
...
$initialize = Invoke-Rpc 'initialize' @{ protocolVersion = '2025-11-25'; clientInfo = @{ name = 'lodestone-adhoc-mine-tree'; version = '1.0' } }
...
Write-Host "Sending minecraft_goal: mine a tree (intelligence=$Intelligence, safety=$Safety)..."
$goalRpc = Invoke-Rpc 'tools/call' @{
    name = 'minecraft_goal'
    arguments = @{
        goal = 'mine a tree'
        taskId = 'survival.collect-wood'
        mode = 'script'
        maxSteps = 32
