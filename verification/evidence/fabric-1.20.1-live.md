...
```text
gradlew.bat :hosts:fabric:mc1_20_1:build --no-daemon
gradlew.bat --no-daemon :hosts:fabric:mc1_20_1:runServer
```
...
Fabric API and Lodestone, created a dedicated 1.20.1 world, and reached the Minecraft `Done`
state.
...
| Native command discovery | Succeeded; 79 root command children returned |
...
| Explicit command authorization | With `LODESTONE_PERMISSIONS=administer-server`, `say lodestone-1201-authorized` returned `OK` and `executed=true`; `communicate` alone is denied |
| Bad token | HTTP 401 |
...
This is representative integration evidence for the long-lived 1.20.1 line, not a full-product
support claim. Player-present tests, client input/UI, block-entity NBT, inventory/container
operations, registry/schematic/vision surfaces, mod-interoperability testing, and other
loader/version rows remain open.
