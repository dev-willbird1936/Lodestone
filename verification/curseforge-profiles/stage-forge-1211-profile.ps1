...
param(
    ...
    [string]$Profile = "$PSScriptRoot\lodestone-forge-1.21.1",
    [string]$Output = "$PSScriptRoot\lodestone-forge-1.21.1-local.zip"
)
...
try {
    ...
    if ($metadata -notmatch '(?m)^modId\s*=\s*"lodestone"' -or
        ...
        throw 'Jar is not a Lodestone Forge 1.21.1 host artifact.'
    }
