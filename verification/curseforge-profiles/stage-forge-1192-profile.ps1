...
param(
    ...
    [string]$Profile = "$PSScriptRoot\lodestone-forge-1.19.2",
    [string]$Output = "$PSScriptRoot\lodestone-forge-1.19.2-local.zip"
)
...
try {
    ...
    if ($metadata -notmatch '(?m)^modId\s*=\s*"lodestone"' -or
        ...
        throw 'Jar is not a Lodestone Forge 1.19.2 host artifact.'
    }
