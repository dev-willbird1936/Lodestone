...
param(
    ...
    [string]$FabricApiJar,
    [string]$Profile = "$PSScriptRoot\lodestone-fabric-1.21.1",
    [string]$Output = "$PSScriptRoot\lodestone-fabric-1.21.1-local.zip"
)
...
try {
    ...
    if ($metadata.depends.minecraft -ne '1.21.1' -or $metadata.depends.fabricloader -ne '0.16.10') {
        throw 'Jar metadata does not target Fabric 1.21.1 / Loader 0.16.10.'
    }
...
if ([string]::IsNullOrWhiteSpace($FabricApiJar)) {
    $cacheRoot = Join-Path $env:USERPROFILE '.gradle\caches\modules-2\files-2.1\net.fabricmc.fabric-api\fabric-api\0.116.9+1.21.1'
    $FabricApiJar = (Get-ChildItem -LiteralPath $cacheRoot -Recurse -Filter 'fabric-api-0.116.9+1.21.1.jar' -File |
        Select-Object -First 1 -ExpandProperty FullName)
}
if ([string]::IsNullOrWhiteSpace($FabricApiJar) -or -not (Test-Path -LiteralPath $FabricApiJar)) {
    throw 'Fabric API 0.116.9+1.21.1 jar not found; pass -FabricApiJar explicitly.'
}
