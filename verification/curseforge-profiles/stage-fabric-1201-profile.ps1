...
param(
    ...
    [string]$FabricApiJar,
    [string]$Profile = "$PSScriptRoot\lodestone-fabric-1.20.1",
    [string]$Output = "$PSScriptRoot\lodestone-fabric-1.20.1-local.zip"
)
...
try {
    ...
    if ($metadata.depends.minecraft -ne '1.20.1' -or $metadata.depends.fabricloader -ne '0.15.11') {
        throw 'Jar metadata does not target Fabric 1.20.1 / Loader 0.15.11.'
    }
